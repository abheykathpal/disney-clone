...
const Slider = () => {
  ...
  const getTrendingMovies = async()=>{
    await GlobalApi.getTrendingVideos().then((resp)=>{
    ...
    <div className='flex overflow-x-auto w-full px-16 py-4 scrollbar-none scroll-smooth' ref={elementRef}>
      {movieList.map((item, index) => (
        <img 
          ...
          alt={item.title} 
          className='min-w-full md:h-[450px] object-fill object-left-top mr-5 rounded-md hover:border-[4px] border-gray-400 transition-all duration-100 ease-in' 
        />
      ))}
    </div>
    </div>
